obj = {
    ...
    find: function () {
        ...
        doQuery('/ky-supplier/supplierManage/queryPreContent?supplierType=' + $("#supplierType").combobox('getValue') + "&queryContent=" + $("#queryContent").val());
    },
    // 添加
    addBox: function () {
        $("#addBox").dialog({
            ...
            success: function (data) {
                ...
                if (data != null && data != '' && data != 'undefined') {
                    if (parseInt(data) == 3) {
                        ...
                        id = $("#table").datagrid('getSelected').id;
                        if (id != null && id != "") {
                            $.messager.confirm('继续操作', '确定转为正式客商吗?',
                                function (r) {
                                    $.ajax({
                                        url: '/ky-supplier/supplierManage/updateById',
                                        type: 'get',
                                        dataType: 'json',
                                        data: {id: id},
                                        success: function (res) {
    ...
    sum: function () {
        $('#addForm').form('submit', {
            onSubmit: function () {
                ...
                if (lag == true) {
                    $.ajax({
                        url: '/ky-supplier/supplierManage/save',
                        type: 'POST',
                        dataType: "json",
    ...
    upload: function () {
        $.ajax({
            type: 'post',
            url: '/ky-supplier/supplierManage/PreImport',
            processData: false,
            cache: false,
            contentType: false,
...
$(function () {
    doQuery('/ky-supplier/supplierManage/queryPreContent');
    $.ajax({
        type: 'get',
        url: '/ky-supplier/supplierManage/getSessionRoleCode',
        success: function (data) {
    ...
    $("#table").datagrid({
        ...
        columns: [[
            ...
            },
            {
                ...
                align: 'center',
                formatter: function (val, row) {
                    j = '<a  id="add" data-id="98" class=" operA"  onclick="obj.updateSupplier(\'' + row.id + '\')">转为正式客商</a> ';
                    return j;
                }
            },
        ]],
