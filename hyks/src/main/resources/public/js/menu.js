...
$(function () {
    // 加载表格
    doQuery('/ky-supplier/menu/queryPage');
})

function doQueryMenuLevel() {
    $("#parentIdCombo").combobox({
        url: '/ky-supplier/menu/queryByParams?isFirstMenu=1',
        method: 'get',
        valueField: 'id',
        textField: 'menuName',
...
function queryCurMostSort(param) {
    ...
    if (typeof param == "number") {
        url = '/ky-supplier/menu/queryCurMostSort?isFirstMenu=' + param;
    } else {
        url = '/ky-supplier/menu/queryCurMostSort?parentId=' + param;
    }
    $.ajax({
        url: url,
        type: 'get',
...
obj = {
    // 查询
    find: function () {
        doQuery('/ky-supplier/menu/queryPage?' + $("#tableFindForm").serialize())
    },
    ...
    edit: function (id) {
        ...
        doQueryMenuLevel();
        $.ajax({
            url: '/ky-supplier/menu/queryById?id=' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $.messager.progress();
    ...
    sum: function () {
        $('#addForm').form('submit', {
            onSubmit: function () {
                ...
                if (lag == true) {
                    $.ajax({
                        url: '/ky-supplier/menu/saveOrUpdate',
                        type: 'POST',
                        dataType: "json",
        ...
        if (rows.length > 0) {
            $.messager.confirm('确定删除', '你确定要删除你选择的记录吗？', function (flg) {
                if (flg) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-supplier/menu/deleteForce?id=" + ids.join(','),
                        beforeSend: function () {
    ...
    delOne: function (id) {
        ...
        $.messager.confirm('提示信息', '是否删除所选择记录', function (flg) {
            if (flg) {
                $.ajax({
                    type: 'get',
                    url: '/ky-supplier/menu/deleteForce?id=' + id,
                    beforeSend: function () {
                        $("#table").datagrid('loading');
                    },
                    success: function (data) {
