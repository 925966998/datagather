...
$(function () {
    // 加载表格
    doQuery('/ky-supplier/sysUser/queryPage');
    doQueryDepartAndRole('companyId', 'role');
})
...
function doQueryDepartAndRole(did, rid) {
    ...
    $("#" + rid).combobox({
        url: '/ky-supplier/role/queryByParams',
        method: 'get',
        valueField: 'id',
        textField: 'roleName'
    });
...
obj = {
    // 查询
    find: function () {
        doQuery('/ky-supplier/sysUser/queryPage?' + $("#tableFindForm").serialize())
    },
    // 添加
    addBox: function () {
        $("#addBox").dialog({
    ...
    edit: function (id) {
        ...
        doQueryDepartAndRole('departmentByDialog', 'roleByDialog');
        $.ajax({
            url: '/ky-supplier/sysUser/queryById?id=' + id,
            type: 'get',
            dataType: 'json',
            success: function (data) {
                ...
                if (data) {
                    ...
                    $('#departmentByDialog').combobox('setValues', data.companyId);
                    $("#userNote").val(data.userNote);
                }
            },
    ...
    sum: function () {
        $('#addForm').form('submit', {
            onSubmit: function () {
                ...
                if (lag == true) {
                    $.ajax({
                        url: '/ky-supplier/sysUser/saveOrUpdate',
                        type: 'POST',
                        dataType: "json",
        ...
        if (rows.length > 0) {
            $.messager.confirm('确定删除', '你确定要删除你选择的记录吗？', function (flg) {
                if (flg) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-supplier/sysUser/deleteForce?id=" + ids.join(','),
                        beforeSend: function () {
                            $("#table").datagrid('loading');
                        },
                        success: function (data) {
    ...
    delOne: function (id) {
        ...
        $.messager.confirm('提示信息', '是否删除所选择记录', function (flg) {
            if (flg) {
                $.ajax({
                    type: 'get',
                    url: '/ky-supplier/sysUser/deleteForce?id=' + id,
                    beforeSend: function () {
                        $("#table").datagrid('loading');
                    },
                    success: function (data) {
