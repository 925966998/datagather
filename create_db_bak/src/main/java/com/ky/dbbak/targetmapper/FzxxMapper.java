...
import com.ky.dbbak.entity.FZXXEntity;
...
public interface FzxxMapper extends BaseMapper {
    ...
    List<FZXXEntity> _queryPage(Map pagerParam);

    /**
     * 根据条件查询全部 参数： map里的key为属性名（字段首字母小写） value为查询的条件，默认为等于 要改动sql请修改 *Mapper 类里的 _query() 方法
     */
    @SelectProvider(type = FzxxSql.class, method = "_queryAll")
    List<FZXXEntity> _queryAll(Map pagerParam);
     ...
     */
    @SelectProvider(type = FzxxSql.class, method = "_get")
    OrgEntity _get(String id);

    /**
     * 删除（逻辑） 参数： id ： 要删除的记录的id
     */
    @DeleteProvider(type = FzxxSql.class, method = "_delete")
    int _delete(String id);

    /**
     * 删除（物理） 参数： id ： 要删除的记录的id
     */
    @DeleteProvider(type = FzxxSql.class, method = "_deleteForce")
     ...
     */
    @InsertProvider(type = FzxxSql.class, method = "_addEntity")
    int _addEntity(FZXXEntity bean);

    /**
     * 更新 参数： id ： 要更新的记录的id 其他map里的参数，key为属性名（字段首字母小写） value为要更新成的值
     */
    @UpdateProvider(type = FzxxSql.class, method = "_update")
