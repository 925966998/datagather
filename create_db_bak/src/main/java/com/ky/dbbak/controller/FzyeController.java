        ...
        for (Map<String, Object> pd : GL_YebList) {
            ...
            for (int i = 1; i < 13; i++) {
                ...
                if (yji !=0 || ydi !=0) {
                    ...
                    List<Map<String, Object>> KjtxxxList = kmxzlxMapper._queryKjtxxx(pd.get("kmdm").toString());
                    dataPull.put("KJTX",KjtxxxList.get(0).get("KJTXDM"));
                    dataPull.put("KJKMBM",pd.get("kmdm"));
                    dataPull.put("KJKMMC",KjtxxxList.get(0).get("kmmc"));
                    String kjkmqc = "";
                    if (pd.get("kmdm") != null) {
                        //12、会计科目全称
                        List<Map<String, Object>> pageDataGL_Ztcs = ztcsMapper._queryZtcs();
                    ...
                    //33.是否最底级科目
                    dataPull.put("SFZDJKM", KjtxxxList.get(0).get("kmmx"));
