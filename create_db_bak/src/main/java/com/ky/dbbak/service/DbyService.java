...
public class DbyService {
    ...
    public List<Map<String, Object>> kjkmResult(List<Map<String, Object>> resultList, Map<String, Object> pageDataGL_Ztcs, int flagVersion) {
...
//    }
    ...
    @Transactional("targetTransactionManager")
    public Boolean kjqjdyService(String KJDZZBBH, int flagVersion) {
        Map<String, Object> pageData = new HashMap<String, Object>();
        List<Map<String, Object>> resultList = new ArrayList<>();
        List<Map<String, Object>> dzzbxxList = new ArrayList<>();
        List<Map<String, Object>> pubKjqjList = new ArrayList<>();
        if (flagVersion == 1) {
            pageData.put("KJDZZBBH", KJDZZBBH);
    ...
    @Transactional("targetTransactionManager")
    public Boolean kmncsService(String KJDZZBBH, int flagVersion) {
        Map<String, Object> pageData = new HashMap<String, Object>();
        List<Map<String, Object>> resultList = new ArrayList<>();
        List<Map<String, Object>> glYebList = new ArrayList<>();
        List<Map<String, Object>> dzzbxxList = new ArrayList<>();
        ...
        for (Map<String, Object> pd : glYebList) {
            ...
            dataPull.put("gsdm", pd.get("gsdm").toString());
            dataPull.put("ZTH", pd.get("ZTH").toString());
            //8.会计月份
            if (new BigDecimal(pd.get("ncd").toString()).compareTo(new BigDecimal("0")) == 0 && new BigDecimal(pd.get("ncj").toString()).compareTo(new BigDecimal("0")) == 0) {
                continue;
            }
            ...
            if (flagVersion == 1) {
                ...
                Map<String, Object> queryPd = new HashMap<String, Object>();
                queryPd.put("kmdm", pd.get("kmdm"));
                queryPd.put("gsdm", pd.get("gsdm").toString());
                queryPd.put("ZTH", pd.get("ZTH").toString());
                queryPd.put("kjnd", pd.get("kjnd").toString());
                pageDataGL_KMXX = kmxxMapper._queryKmxxmx(queryPd);
            }
            if (pageDataGL_KMXX != null && pageDataGL_KMXX.size() > 0) {
                dataPull.put("KJKMMC", pageDataGL_KMXX.get(0).get("kmmc").toString().trim().replace("　", ""));
        ...
        if (flagVersion == 1) {
            resultListNew = kjkmResult(resultList, pageDataGL_Ztcs.get(0), 1);

        } else if (flagVersion == 2) {
            resultListNew = kjkmResult(resultList, pageDataGL_Ztcs.get(0), 2);
    ...
    @Transactional("targetTransactionManager")
    public Boolean kmyeService(String KJDZZBBH, int flagVersion) {
        Map<String, Object> pageData = new HashMap<String, Object>();
        List<Map<String, Object>> resultList = new ArrayList<>();
        ...
        if (flagVersion == 1) {
            GL_YebList = yebMapper._queryAllYeb(pageData);
            pageData.put("KJDZZBBH", KJDZZBBH);
            dzzbxxList = dzzbxxMapper._queryDzzbxx(pageData);
            pageDataGL_Ztcs = ztcsMapper._queryZtcs();
        } else if (flagVersion == 2) {
        ...
        for (Map<String, Object> pd : GL_YebList) {
            ...
            dataPullBase.put("QMJFYE", BigDecimal.ZERO);
            dataPullBase.put("QMDFYE", BigDecimal.ZERO);
            dataPullBase.put("gsdm", pd.get("gsdm").toString());
            dataPullBase.put("ZTH", pd.get("ZTH").toString());
            BigDecimal jfljfse = new BigDecimal("0");
            BigDecimal dfljfse = new BigDecimal("0");
            BigDecimal qmjfye = new BigDecimal("0");
            BigDecimal qmdfye = new BigDecimal("0");
            ...
            for (int i = 1; i < 13; i++) {
                ...
                long num = 0L;
                if (flagVersion == 1) {
                    num = pznrMapper._queryByPznrCount(qCountPd);
                } else if (flagVersion == 2) {
                    qCountPd.put("gsdm", pd.get("gsdm").toString());
                    qCountPd.put("ZTH", pd.get("ZTH").toString());
                    num = pznrMapper._queryByPznrCount(qCountPd);
    ...
    @Transactional("targetTransactionManager")
    public int jzpzService(String KJDZZBBH, int flagVersion) {
        Map<String, Object> pageData = new HashMap<String, Object>();
        Map<String, List<Map<String, Object>>> jzpznrList = new HashMap<String, List<Map<String, Object>>>();
        List<Map<String, Object>> resultList = new ArrayList<>();
        List<Map<String, Object>> resultAllList = new ArrayList<>();
        List<Map<String, Object>> bypznrList = new ArrayList<>();
        List<Map<String, Object>> dzzbxxList = new ArrayList<>();
        List<Map<String, Object>> pageDataGL_Ztcs = new ArrayList<>();
        if (flagVersion == 1) {
            bypznrList = pznrMapper._queryAll(pageData);
            pageData.put("KJDZZBBH", KJDZZBBH);
            dzzbxxList = dzzbxxMapper._queryDzzbxx(pageData);
            pageDataGL_Ztcs = ztcsMapper._queryZtcs();
...
//                dataPull.put("JZLXBH", pd.get("PZLXDM").toString());
                //11.记账类型名称
                List<Map<String, Object>> pageDatePzlxList = new ArrayList<>();
                if (flagVersion == 1) {
                    pageDatePzlxList = pzlxMapper._queryPzlx(pd);
                } else if (flagVersion == 2) {
                    pageData.put("pzlxdm", pd.get("PZLXDM").toString());
                    pageDatePzlxList = pzlxMapper._queryPzlxG(pageData);
                }
                String pzlxmc = "";
                if (pageDatePzlxList != null && pageDatePzlxList.size() > 0) {
                    pzlxmc = pageDatePzlxList.get(0).get("pzlxmc").toString();
...
//                String kmmc = pageDataGL_KMXX.get(0).get("kmmc").toString();
                ...
                if (!StringUtils.isEmpty(fzdm4)) {
                    ...
                    //45.功能科目名称
                    dataFzxlbMap.put("fzdm", fzdm4);
                    List<Map<String, Object>> fzxzlList = new ArrayList<>();
                    if (flagVersion == 1) {
                        fzxzlList = glFzxzlMapper._queryFzdm(dataFzxlbMap);
                    } else if (flagVersion == 2) {
                        dataFzxlbMap.put("fzdm", fzdm4);
                        dataFzxlbMap.put("gsdm", pd.get("gsdm").toString());
                        fzxzlList = glFzxzlMapper._queryFzdm(dataFzxlbMap);
                ...
                if (!fzdm4.equals("") && !StringUtils.isEmpty(fzdm5)) {
                    ...
                    List<Map<String, Object>> FzxzlList = new ArrayList<>();
                    if (flagVersion == 1) {
                        FzxzlList = glFzxzlMapper._queryGL_Fzxzl(dataFzxlbMap);
                    } else if (flagVersion == 2) {
                        dataFzxlbMap5.put("fzdm", fzdm5);
                        dataFzxlbMap5.put("gsdm", pd.get("gsdm").toString());
                        //47.经济科目名称
