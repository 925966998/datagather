...
public interface GlztcsMapper extends BaseMapper {
     ...
     */
    @SelectProvider(type = GlztcsSql.class, method = "_queryPage")
    ...
    @SelectProvider(type = GlztcsSql.class, method = "_queryAll")
    List<AreaEntity> _queryAll(Map pagerParam);

    /**
     * 根据条件查询总条数 map里的key为属性名（字段首字母小写） value为查询的条件，默认为等于 要改动sql请修改 *Mapper 类里的 _query() 方法
     */
    @SelectProvider(type = GlztcsSql.class, method = "_queryCount")
    long _queryCount(Map params);

    /**
     * 按id查询 参数： id ： 要查询的记录的id
     */
    @SelectProvider(type = GlztcsSql.class, method = "_get")
    AreaEntity _get(String id);

    /**
     * 删除（逻辑） 参数： id ： 要删除的记录的id
     */
    @DeleteProvider(type = GlztcsSql.class, method = "_delete")
    int _delete(String id);

    /**
     * 删除（物理） 参数： id ： 要删除的记录的id
     */
    @DeleteProvider(type = GlztcsSql.class, method = "_deleteForce")
     ...
     */
    @SuppressWarnings("rawtypes")
    @InsertProvider(type = GlztcsSql.class, method = "_add")
    int _add(Map params);

    /**
     * 按实体类新增 参数： 实体类对象，必须有id属性
     */
    @InsertProvider(type = GlztcsSql.class, method = "_addEntity")
    int _addEntity(AreaEntity bean);

    /**
     * 更新 参数： id ： 要更新的记录的id 其他map里的参数，key为属性名（字段首字母小写） value为要更新成的值
     */
    @InsertProvider(type = GlztcsSql.class, method = "_update")
    int _update(Map params);

    @Select("select top 1 Kmbmfa as kmbmfa  from dbo.GL_Ztcs where kjnd=#{kjnd} and ztbh!='99999999999999999999'")
    String queryKjkmjg(String kjnd);
}
