...
function sendSever(){
	...
	//加密方式  (取其中一个参数名+当前日期[格式 20150405]+混淆码",ly," 然后md5加密 的值作为 参数FKEY的值提交)
	var paraname = $("#S_TYPE_S").val();	//要加密的参数
	var nowtime = date2str(new Date(),"yyyyMMdd");
	//alert($.md5(paraname+nowtime+',ly,'));
	...
	$.ajax({
    	...
    	data: {serverUrl:$("#serverUrl").val()+"&FKEY="+$.md5(paraname+nowtime+',ly,'),requestMethod:$("#S_TYPE").val(),tm:new Date().getTime()},
		dataType:'json',
		cache: false,
