...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="happuser/listUsers" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendEmail(EMAIL){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '<%=basePath%>head/goSendEmail?EMAIL='+EMAIL+'&msg=appuser';
			 diag.Width = 660;
			 diag.Height = 470;
		...
		function sendSms(phone){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送短信";
			 diag.URL = '<%=basePath%>head/goSendSms?PHONE='+phone+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 265;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增会员";
			 diag.URL = '<%=basePath%>happuser/goAddU';
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = '<%=basePath%>happuser/goEditU?USER_ID='+user_id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>happuser/deleteU?USER_ID="+userId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>happuser/deleteAllU?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>happuser/excel?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID+'&STATUS='+STATUS;
		}
		</script>
