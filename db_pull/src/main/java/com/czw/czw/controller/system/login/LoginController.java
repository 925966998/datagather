...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").split(",ly,");
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){
				...
				pd.put("USERNAME", USERNAME);
				//if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){//关闭验证码
...
//		pd.put("SYSNAME", Tools.readTxtFile(Const.SYSNAME)); //读取系统名称
	...
	public ModelAndView defaultPage() throws Exception{
		...
		ModelAndView mv = this.getModelAndView();
		int sellCount =0; 
		mv.addObject("sellCount",sellCount);//当天销售的订单
		
		int state=pd.getInt("sel");
		if(state==2){
			pd.put("nextTime", DateUtil.getDay().concat(" 00:00:00"));
			pd.put("endTime", DateUtil.getDay().concat(" 23:59:59"));
		}
		if(state==3){
			pd.put("nextTime", DateUtil.getMonth(new Date()).concat("-01 00:00:00"));
			pd.put("endTime", DateUtil.getMonth(new Date()).concat("-31 23:59:59"));
		}else{
		}
////		//支付方式 1支付宝 2 微信 3 银联 4 线下
////				double zfbTotal=0;
////				double wxTotal=0;
////				double ylTotal=0;
////				double xxTotal=0;
...
////					xxTotal=0.0;
////				}
////				total=total+zfbTotal+wxTotal+ylTotal+xxTotal;
//				mv.addObject("zfbTotal", zfbTotal);
//				mv.addObject("wxTotal", wxTotal);
//				mv.addObject("ylTotal", ylTotal);
//				mv.addObject("xxTotal", xxTotal);
//				mv.addObject("total", total);
				mv.addObject("sel", state);
	...
	public ModelAndView logout(){
		...
		session.removeAttribute(Const.WARE_ID);
		session.removeAttribute(Const.TOUSERTYPE);
		session.removeAttribute("changeMenu");
