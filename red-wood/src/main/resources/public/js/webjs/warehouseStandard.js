...
$(function () {
    doQuery('/ky-redwood/process/queryStandardPage');
    $("#tabelShowBox").hide();
});
obj = {
    // 查询
    find: function () {
        doQuery('/ky-redwood/process/queryQualityPage?' + $("#tableFindForm").serialize())
    },
    // 添加
    addBox: function () {
        $("#addBox").dialog({
    ...
    addStandard: function () {
        ...
        console.log(rows);
        if (rows.length > 0) {
            $.messager.confirm('确定质检通过', '你确定要通过你选择的商品吗？', function (flg) {
                if (flg) {
                    var ids = [];
                    for (i = 0; i < rows.length; i++) {
                        ids.push(rows[i].id);
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-redwood/process/addStandard?id=" + ids.join(','),
                        beforeSend: function () {
                            $("#table").datagrid('loading');
                        },
                        success: function (data) {
            ...
            $.messager.alert('提示', '请选择要质检的商品', 'info');
        }
    },
    // 审核不通过
    updateStandard: function () {
        ...
        console.log(rows);
        if (rows.length > 0) {
            $.messager.confirm('确定审核不通过', '你确定要拒绝你选择的商品吗？', function (flg) {
                if (flg) {
                    var ids = [];
                    for (i = 0; i < rows.length; i++) {
                        ids.push(rows[i].id);
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-redwood/process/updateStandard?id=" + ids.join(','),
                        beforeSend: function () {
                            $("#table").datagrid('loading');
                        },
                        success: function (data) {
