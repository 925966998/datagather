...
$(function () {
    doQuery('/ky-redwood/product/queryPage?productStatus='+0);
    $("#tabelShowBox").hide();
});
obj = {
    // 查询
    ...
    out: function () {
        ...
        console.log(rows);
        if (rows.length > 0) {
            $.messager.confirm('确定出库', '你确定要出库你选择的商品吗？', function (flg) {
                if (flg) {
                    var ids = [];
                    for (i = 0; i < rows.length; i++) {
                        ids.push(rows[i].id);
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-redwood/product/outForce?id=" + ids.join(','),
                        beforeSend: function () {
                            $("#table").datagrid('loading');
                        },
                        success: function (data) {
            ...
            $.messager.alert('提示', '请选择要出库的商品', 'info');
        }
    },
}
